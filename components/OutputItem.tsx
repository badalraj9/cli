...
export const OutputItem: React.FC<OutputItemProps> = ({ item }) => {
  ...
  // Memoize components to prevent re-mounting on every render/stream update
  const markdownComponents = useMemo(() => ({
    ...
    ul: ({ children }: any) => <ul className="list-disc list-outside my-4 ml-4 space-y-2 text-claude-dim">{children}</ul>,
    ol: ({ children }: any) => <ol className="list-decimal list-outside my-4 ml-4 space-y-2 text-claude-dim">{children}</ol>,
    a: ({ href, children }: any) => (
    ...
    h1: ({ children }: any) => <h1 className="text-xl font-bold text-white mb-4 mt-8 font-sans">{children}</h1>,
    h2: ({ children }: any) => <h2 className="text-lg font-bold text-white mb-3 mt-6 font-sans">{children}</h2>,
    h3: ({ children }: any) => <h3 className="text-base font-semibold text-white mb-2 mt-4 font-sans">{children}</h3>,
  }), []);
